...
function toolSpans(event, { deriveControlSpans, agentName, userMetadata }) {
  ...
  // The redacted command is always on the event; the tool's stdout/stderr is only
  // present when content capture is on (LLM_CAPTURE_CONTENT). Map both onto the OTel
  // GenAI message arrays so AO renders execute_tool input/output.
  const argvRedacted = Array.isArray(event.command?.argv_redacted) ? event.command.argv_redacted : [];
  const commandText = argvRedacted.length ? argvRedacted.join(" ") : null;
  const outputText = event.content?.output ?? null;
  ...
  spans.push(
    span({
      ...
      attributes: {
        ...
        // Tool-native I/O: AO reads gen_ai.tool.call.{arguments,result} first for a
        // tool span. The gen_ai.*.messages pair is kept as a fallback + for chat-shaped
        // consumers. arguments = the run command; result = its output (when captured).
        "gen_ai.tool.call.arguments": commandText,
        "gen_ai.tool.call.result": outputText,
        "gen_ai.input.messages": commandText
          ? JSON.stringify([{ role: "user", content: commandText }])
          : null,
        "gen_ai.output.messages": outputText
